...
public class Gif {
  ...
  private GifDecoder gd= new GifDecoder();
  private int framec;
  ...
  public void changeTarget(String f) {
    gd.read(f);
    ...
    img=buf_to_img(gd.getImage());
  }

  public PImage getThumbNail() {
    return buf_to_img(gd.getImage());
  }

  public void imagegif(int x, int y) {
    if (millis()-prevt>del) {
      i++;
      prevt=millis();
      del=gd.getDelay((i-1+buffsize)%framec);
      buff[(i-1+buffsize)%buffsize]=buf_to_img(gd.getFrame((i-1+buffsize)%framec));
    }
    image(buff[(i-1)%buffsize], x, y, img.width*resize_x, img.height*resize_y);
  }
  public void imagegif(int x, int y, int dx, int dy) {
    if (millis()-prevt>del) {
      i++;
      prevt=millis();
      del=gd.getDelay((i-1+buffsize)%framec);
    ...
    image(buff[(i-1)%buffsize], x, y, dx*resize_x, dy*resize_y);
  }


  private PImage buf_to_img(BufferedImage val) {
    try {
  ...
  public void resize(int wid, int hei) {
    if (hei==0) {
      resize_x=wid/img.width;
      resize_y=resize_x;
    } else if (wid==0) {
      resize_y=hei/img.height;
      resize_x=resize_y;
    } else {
      resize_x=wid/img.width;
      resize_y=hei/img.height;
    ...
    protected void setPixels() {
      ...
      if (lastDispose > 0) {
        ...
        if (lastImage != null) {
          ...
          //println(prev.length+" "+dest.length+" "+width*height+" ");
